...
def graphlet_occurrence_distance_to_nodes(g: nx.Graph, nodes_shortest_path_lookup: Dict[str, Dict[str, int]], group: List[str]):
    path_lengths = []
...
) -> Tuple[GraphletOccurrence, GraphletPositionalMetrics]:
    return (
        graphlet_occurrence,
        GraphletPositionalMetrics(
            ...
            anchor_node_distances=graphlet_occurrence_distance_to_nodes(
                g, nodes_shortest_path_lookup, graphlet_occurrence.nodes
            ),
            graph_module_participation=graphlet_occurrence_module_participation(
                g, graph_modules, graphlet_occurrence.nodes
            ),
        ),
    )
...
def process_graphlet_occurrences(
    ...
    with Pool(processes=WORKERS) as p:
        with tqdm(
            ...
            for (g_oc, g_pm) in p.imap_unordered(
